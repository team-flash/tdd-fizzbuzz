#!/bin/bash
# https://github.com/ConSol/docker-headless-vnc-container/blob/master/src/common/scripts/vnc_startup.sh
# modified to disable password
...
if [[ $VNC_VIEW_ONLY == "true" ]]; then
    VIEW_ONLY_ARGS="-AlwaysShared -AcceptPointerEvents=false AcceptKeyEvents=false"
else
    VIEW_ONLY_ARGS=""
fi
vncserver $DISPLAY -depth $VNC_COL_DEPTH -geometry $VNC_RESOLUTION $VIEW_ONLY_ARGS -SecurityTypes None,TLSNone &> $STARTUPDIR/no_vnc_startup.log
